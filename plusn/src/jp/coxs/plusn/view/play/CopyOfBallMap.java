...
public class CopyOfBallMap {
     ...
     * コンストラクター
     */
    public CopyOfBallMap(StageInfoBean stageInfo) {
...
//        } while (getClearBallList(x, y).size() > 0 && count < 100);
        ...
        for (int i = 0; i < 7; i++) {
            ...
            for (int j = i; j < 7; j++) {
                if (col.get(j) < 1 || col.get(j) > 20) {
                    // タイルなしの場合、合計値をリセット
                    j = 7;
                } else if (col.get(j) == 10) {
                    if (sumx == 0) {
                        j = 7;
                    }
                } else if (col.get(j) == 11) {
                    // タイルなしの場合、合計値をリセット
                    j = 7;
                } else {
                    sumx += col.get(j);
                ...
                if (j < 7 && sumx % 10 == 0) {
                    if (i <= y && j >= y) {
                        ...
                        if (i > 0 && col.get(i - 1) == 11) {
                            PointBean rootPoint = new PointBean(x, i - 1);
                            rootPoint.setValue(ballMap[x][i - 1]);
                            // ポイントをクリアリストに追加する
                            clearPointList.add(rootPoint);
                        }
                        if (j < 6 && col.get(j + 1) == 11) {
                            PointBean rootPoint = new PointBean(x, j + 1);
                            rootPoint.setValue(ballMap[x][j + 1]);
                            // ポイントをクリアリストに追加する
                            clearPointList.add(rootPoint);
                        }
                    }
        ...
        for (int i = 0; i < 7; i++) {
            ...
            for (int j = i; j < 7; j++) {
                ...
                if (row.get(j) < 1 || row.get(j) > 20) {
                    // タイルなしの場合、合計値をリセット
                    j = 7;
                } else if (row.get(j) == 10) {
                    if (sumy == 0) {
                        j = 7;
                    }
                } else if (row.get(j) == 11) {
                    // タイルなしの場合、合計値をリセット
                    j = 7;
                } else {
                    sumy += row.get(j);
                ...
                if (j < 7 && sumy % 10 == 0) {
                    if (i <= x && j >= x) {
                        ...
                        if (i > 0 && row.get(i - 1) == 11) {
                            PointBean rootPoint = new PointBean(i - 1, y);
                            rootPoint.setValue(ballMap[i - 1][y]);
                            // ポイントをクリアリストに追加する
                            clearPointList.add(rootPoint);
                        }
                        if (j < 6 && row.get(j + 1) == 11) {
                            PointBean rootPoint = new PointBean(j + 1, y);
                            rootPoint.setValue(ballMap[j + 1][y]);
                            // ポイントをクリアリストに追加する
                            clearPointList.add(rootPoint);
                        }
                    }
    ...
    public int getScoreType(int x, int y) {
        ...
        //
        List<Integer> col = new ArrayList<Integer>();
        col.add(ballMap[x][0] % 10);
        col.add(ballMap[x][1] % 10);
        col.add(ballMap[x][2] % 10);
        col.add(ballMap[x][3] % 10);
        col.add(ballMap[x][4] % 10);
        col.add(ballMap[x][5] % 10);
        col.add(ballMap[x][6] % 10);
        ...
        //
        List<Integer> row = new ArrayList<Integer>();
        row.add(ballMap[0][y] % 10);
        row.add(ballMap[1][y] % 10);
        row.add(ballMap[2][y] % 10);
        row.add(ballMap[3][y] % 10);
        row.add(ballMap[4][y] % 10);
        row.add(ballMap[5][y] % 10);
        row.add(ballMap[6][y] % 10);
    ...
    public BallClearBean getClearBallList(int x, int y, int i, String string) {
        ...
        // ボール0やボールNの場合空リストを戻す
        if (ballMap[x][y] == 10 && ballMap[x][y] == 11) {
        ...
        for (int j = i; j < 7; j++) {
            if (valueList.get(j) < 1 || valueList.get(j) > 20) {
                // タイルなしの場合、合計値をリセット
                j = 7;
            } else if (valueList.get(j) == 10) {
                if (sumx == 0) {
                    j = 7;
                }
            } else if (valueList.get(j) == 11) {
                // タイルなしの場合、合計値をリセット
                j = 7;
            } else {
            ...
            if (j < 7 && sumx % 10 == 0) {
                if ("x".equals(string)) {
                    if (i <= y && j >= y) {
                        ...
                        if (i > 0 && valueList.get(i - 1) == 11) {
                            PointBean rootPoint = new PointBean(x, i - 1);
                            rootPoint.setValue(ballMap[x][i - 1]);
                            // ポイントをクリアリストに追加する
                            clearPointList.add(rootPoint);
                        }
                        if (j < 6 && valueList.get(j + 1) == 11) {
                            PointBean rootPoint = new PointBean(x, j + 1);
                            rootPoint.setValue(ballMap[x][j + 1]);
                            // ポイントをクリアリストに追加する
                            clearPointList.add(rootPoint);
